    ...
    beforeUnmount() {
        ...
        window.addEventListener("resize", () => {
            ['chart-posts', 'chart-comments', 'chart-categories'].forEach(id => {
                ...
                if (dom) echarts.getInstanceByDom(dom)?.resize();
            });
        });
    },
    methods: {
        ...
        async openPost(postId) {
            const postInCache = this.defaultHomepagePostsCache.find(p => p.id === postId);
            if (postInCache) postInCache.viewCount = Number(postInCache.viewCount || 0) + 1;

            const postInList = this.posts.find(p => p.id === postId);
            if (postInList) postInList.viewCount = Number(postInList.viewCount || 0) + 1;
        ...
        async likePost() {
            ...
            const data = await this.request(`/api/posts/${this.selectedPostId}/like`, {
                method: "POST"
            }, "点赞成功！", null, false); // 根据后端代码，点赞并未强制要求传递 currentUser

            if (data) {
                // 更新当前详情页的数据（点赞总数会随之更新并累加显示）
                this.selectedPost = normaliseDetail(data);

                // 同步更新缓存和列表中的点赞数，以保证返回首页时数据一致
                const postInCache = this.defaultHomepagePostsCache.find(p => p.id === this.selectedPostId);
                if (postInCache) postInCache.likeCount = data.likeCount;

                const postInList = this.posts.find(p => p.id === this.selectedPostId);
                if (postInList) postInList.likeCount = data.likeCount;
            }
        },

        async inspectCategory(categoryId) {
        ...
        syncCommentCreated(commentPayload) {
            if (!this.selectedPostId) {
                return;
            }

            const postId = String(this.selectedPostId);
            const comment = commentPayload?.comment || commentPayload;
            const rawCommentCount = commentPayload?.commentCount;
            const serverCommentCount = rawCommentCount == null ? NaN : Number(rawCommentCount);
            const existingComments = Array.isArray(this.selectedPost?.comments)
                ? [...this.selectedPost.comments]
                : [];
            const commentExists = comment?.id != null
                && existingComments.some((item) => String(item.id) === String(comment.id));
            const nextCommentCount = Number.isFinite(serverCommentCount)
                ? serverCommentCount
                : Number(this.selectedPost?.commentCount || 0) + (commentExists ? 0 : 1);

            if (this.selectedPost && String(this.selectedPost.id) === postId) {
                if (!commentExists) {
                    existingComments.push(comment);
                }
                this.selectedPost.comments = existingComments;
                this.selectedPost.commentCount = nextCommentCount;
            }

            const syncPostList = (posts) => {
            ...
            syncPostList(this.posts);
            syncPostList(this.defaultHomepagePostsCache);
            syncPostList(this.myPosts);
        },
        async submitComment() {
            if (!this.currentUser) {
        ...
        renderPostLineChart() {
            ...
            myChart.setOption({
                ...
                series: [{ data: data, type: 'line', smooth: true, itemStyle: { color: '#8f4b2f' } }]
            });
        },
        renderCommentLineChart() {
            ...
            // 注：由于前端没有单条评论的日期流，此处统计"按文章发布日期的评论总数分布"
            const dateCounts = {};
            this.defaultHomepagePostsCache.forEach(post => {
                const date = post.createdAt.substring(0, 10);
            ...
            myChart.setOption({
                ...
                xAxis: { type: 'category', data: sortedDates },
                yAxis: { type: 'value', minInterval: 1 },
                series: [{ data: data, type: 'line', smooth: true, itemStyle: { color: '#6d7561' } }]
            });
        },
        renderCategoryPieChart() {
            ...
            if (!chartDom) return;
            const myChart = echarts.init(chartDom);
            const pieData = this.categoryStats.filter(s => s.count > 0).map(stat => ({
